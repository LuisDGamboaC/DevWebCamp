...
class ActiveRecord {
    ...
    //Retornar los registros por un orden = asc desc
    ...
    public static function wherArray($array = []) {
        $query = "SELECT * FROM " . static::$tabla . " WHERE ";
        foreach($array as $key => $value) {
            if($key == array_key_last($array)){ // le muestra el ultimo elemento de un arreglo
                $query .= " {$key} = '{$value}' ";
            }else {
                $query .= " {$key} = '{$value}' AND";
            }
    ...
    public static function total($columna = '', $valor = '') {
        ...
        return array_shift($total); // solo saca el valor que es 18 en total
    }
